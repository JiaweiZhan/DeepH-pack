...
class Transform:
    ...
    def state_dict(self):
        result = {"normalizer": self.normalizer, "boxcox": self.boxcox}
        if self.normalizer:
...
def save_model(state, model_dict, model_state_dict, path, is_best):
    ...
    if is_best:
        shutil.copyfile(
            os.path.join(path, "model.pt"), os.path.join(path, "best_model.pt")
        )
        shutil.copyfile(
            os.path.join(path, "state_dict.pkl"),
            os.path.join(path, "best_state_dict.pkl"),
        )
...
def get_config(args):
    ...
    if config["basic"]["target"] == "O_ij":
        assert config["basic"]["O_component"] in [
            "H_minimum",
            "H_minimum_withNA",
            "H",
            "Rho",
    ...
    if config["basic"]["target"] == "E_ij":
        assert config["basic"]["energy_component"] in [
            "xc",
            "delta_ee",
            "both",
            "summation",
    ...
    assert config["basic"]["interface"] in [
        "h5",
        "h5_rc_only",
        "h5_Eij",
        "npz",
    ...
    assert config["network"]["distance_expansion"] in [
        "GaussianBasis",
        "BesselBasis",
        "ExpBernsteinBasis",
    ]
    assert config["network"]["normalization"] in [
        "BatchNorm",
        "LayerNorm",
        "PairNorm",
        "InstanceNorm",
    ...
    assert config["network"]["atom_update_net"] in ["CGConv", "GAT", "PAINN", "GINE"]
    assert config["hyperparameter"]["optimizer"] in [
        "sgd",
        "sgdm",
        "adam",
        "adamW",
    ...
    assert config["hyperparameter"]["lr_scheduler"] in [
        "",
        "MultiStepLR",
        "ReduceLROnPlateau",
        "CyclicLR",
...
def get_inference_config(*args):
    config = ConfigParser()
    config.read(
        os.path.join(os.path.dirname(__file__), "inference", "inference_default.ini")
    )
    for config_file in args:
        config.read(config_file)
...
def get_preprocess_config(*args):
    config = ConfigParser()
    config.read(
        os.path.join(os.path.dirname(__file__), "preprocess", "preprocess_default.ini")
    )
    for config_file in args:
        config.read(config_file)
    ...
    assert if_integer(
        config["basic"]["multiprocessing"]
    ), "value of multiprocessing must be an integer"
