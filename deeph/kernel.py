...
class DeepHKernel:
    def __init__(self, config: ConfigParser):
        ...
        if self.if_tensorboard:
            self.tb_writer = SummaryWriter(
                os.path.join(config.get("basic", "save_dir"), "tensorboard")
            )
        ...
        if not config.getboolean("basic", "disable_cuda"):
            self.device = torch.device(
                config.get("basic", "device") if torch.cuda.is_available() else "cpu"
            )
        else:
        ...
        if config.get("hyperparameter", "dtype") == "float32":
            ...
            raise ValueError(
                "Unknown dtype: {}".format(config.get("hyperparameter", "dtype"))
            )
        np.seterr(all="raise")
        ...
        if config.getint("basic", "num_threads", fallback=-1) == -1:
            if torch.cuda.device_count() == 0:
                ...
                torch.set_num_threads(
                    cpu_count(logical=False) // torch.cuda.device_count()
                )
        else:
        ...
        # early_stopping
        self.early_stopping_loss_epoch = json.loads(
            self.config.get("train", "early_stopping_loss_epoch")
        )
    ...
    def build_model(self, model_pack_dir: str = None, old_version=None):
        if model_pack_dir is not None:
            ...
            if old_version is True:
                ...
                imp = package.PackageImporter(
                    os.path.join(model_pack_dir, "best_model.pt")
                )
                checkpoint = imp.load_pickle(
                    "checkpoint", "model.pkl", map_location=self.device
                )
                ...
                print(
                    f"=> Atomic types: {self.index_to_Z.tolist()}, "
                ...
                if self.target != "E_ij":
                    ...
                    if self.energy_component == "both":
                        ...
                        raise ValueError(
                            "Unknown energy_component: {}".format(self.energy_component)
                        )
                return checkpoint
        else:
        ...
        model_kwargs = dict(
            ...
            nblock=self.config.getint("network", "nblock"),
            ...
            atom_update_net=self.config.get(
                "network", "atom_update_net", fallback="CGConv"
            ),
            separate_onsite=self.separate_onsite,
            num_l=self.config.getint("network", "num_l"),
            trainable_gaussians=self.config.getboolean(
                "network", "trainable_gaussians", fallback=False
            ),
            type_affine=self.config.getboolean(
                "network", "type_affine", fallback=False
            ),
            if_fc_out=False,
        )
        ...
        if "num_elements" in parameter_list:
            model_kwargs["num_elements"] = (
                self.config.getint("basic", "max_element") + 1
            )
        self.model = HGNN(**model_kwargs)
    ...
    def set_train(self):
        self.criterion_name = self.config.get(
            "hyperparameter", "criterion", fallback="MaskMSELoss"
        )
        if self.target == "E_i":
            self.criterion = MSELoss()
        elif self.target == "E_ij":
            self.criterion = MSELoss()
            self.retain_edge_fea = self.config.getboolean(
                "hyperparameter", "retain_edge_fea"
            )
        ...
        if self.config.get("hyperparameter", "lr_scheduler") == "":
            ...
            lr_milestones = json.loads(
                self.config.get("hyperparameter", "lr_milestones")
            )
            self.scheduler = MultiStepLR(
                self.optimizer, milestones=lr_milestones, gamma=0.2
            )
            ...
            raise ValueError(
                "Unknown lr_scheduler: {}".format(
                    self.config.getfloat("hyperparameter", "lr_scheduler")
                )
            )
        self.load_resume()
    ...
    def load_pretrained(self):
        ...
        if pretrained:
            if os.path.isfile(pretrained):
                ...
                print(
                    f'=> loaded pretrained model at "{pretrained}" (epoch {checkpoint["epoch"]})'
                )
            else:
    ...
    def get_dataset(self, only_get_graph=False):
        dataset = HData(
            ...
            create_from_DFT=self.config.getboolean(
        ...
        print(
            f"number of test set: {len(indices[train_size + val_size:train_size + val_size + test_size])}"
        )
        train_sampler = SubsetRandomSampler(indices[:train_size])
        val_sampler = SubsetRandomSampler(indices[train_size : train_size + val_size])
        test_sampler = SubsetRandomSampler(
            indices[train_size + val_size : train_size + val_size + test_size]
        )
        train_loader = DataLoader(
            dataset,
            batch_size=self.config.getint("hyperparameter", "batch_size"),
            shuffle=False,
        ...
        if self.config.getboolean("basic", "statistics"):
            ...
            plt.tight_layout()
            plt.savefig(
                os.path.join(self.config.get("basic", "save_dir"), "mean.png"), dpi=800
            )
            np.savetxt(
                os.path.join(self.config.get("basic", "save_dir"), "mean.dat"),
                mean_value.numpy(),
            )

            print(
                f"The statistical results are saved to {os.path.join(self.config.get('basic', 'save_dir'), 'mean.dat')}"
            )
        ...
        if normalizer == False and boxcox == False:
            ...
            sample_mask = torch.cat([dataset[i].mask for i in range(len(dataset))])
            transform = Transform(
                sample_label, mask=sample_mask, normalizer=normalizer, boxcox=boxcox
            )
    ...
    def make_mask(self, dataset):
        ...
        for data in dataset:
            if (
                ...
                or self.target == "phiVdphi"
                or self.target == "density_matrix"
            ):
            ...
            label = torch.zeros(
                data.edge_attr.shape[0], out_fea_len, dtype=torch.get_default_dtype()
            )
            ...
            for index_out, orbital_dict in enumerate(self.orbital):
                for N_M_str, a_b in orbital_dict.items():
                    ...
                    if self.spinful:
                        if self.target == "phiVdphi":
                            raise NotImplementedError(
                                "Not yet have support for phiVdphi"
                            )
                        else:
                    ...
                    if if_only_rc == False:
                        if self.spinful:
                            if self.target == "phiVdphi":
                                ...
                                label[:, 8 * index_out : 8 * (index_out + 1)] = (
                                    torch.where(
                                        (
                                            atomic_number_edge_i
                                            == condition_atomic_number_i
                                        )
                                        & (
                                            atomic_number_edge_j
                                            == condition_atomic_number_j
                                        ),
                                        Oij_value[
                                            :, condition_orbital_i, condition_orbital_j
                                        ].t(),
                                        torch.zeros(
                                            8,
                            ...
                            if self.target == "phiVdphi":
                                label[:, 3 * index_out : 3 * (index_out + 1)] = (
                                    torch.where(
                                        (
                                            atomic_number_edge_i
                                            == condition_atomic_number_i
    ...
    def train(self, train_loader, val_loader, test_loader):
        ...
        try:
            for epoch in range(self.config.getint("train", "epochs")):
                if self.config.getboolean(
                    "train", "switch_sgd"
                ) and epoch == self.config.getint("train", "switch_sgd_epoch"):
                    model_parameters = filter(
                        lambda p: p.requires_grad, self.model.parameters()
                    )
                    self.optimizer = optim.SGD(
                        model_parameters,
                        lr=self.config.getfloat("train", "switch_sgd_lr"),
                    )
                ...
                if self.if_tensorboard:
                    self.tb_writer.add_scalar(
                        "Learning rate", learning_rate, global_step=epoch
                    )
                ...
                if self.if_tensorboard:
                    self.tb_writer.add_scalars(
                        "loss", {"Train loss": train_losses.avg}, global_step=epoch
                    )
                ...
                if (
                    val_losses.avg
                    > self.config.getfloat("train", "revert_threshold")
                    * self.best_val_loss
                ):
                    print(
                        f"Epoch #{epoch:01d} \t| "
                        ...
                        f"Train loss: {train_losses.avg:.10f} \t| "
                        f"Val loss: {val_losses.avg:.10f} \t| "
                        f"Best val loss: {self.best_val_loss:.10f}."
                    )
                    best_checkpoint = torch.load(
                        os.path.join(
                            self.config.get("basic", "save_dir"), "best_state_dict.pkl"
                        )
                    )
                    ...
                    if self.config.getboolean("train", "revert_then_decay"):
                        ...
                        if (
                            lr_step < lr_step_num
                            and epoch >= revert_decay_epoch[lr_step]
                        ):
                            for param_group in self.optimizer.param_groups:
                                param_group["lr"] = (
                                    learning_rate * revert_decay_gamma[lr_step]
                                )
                            if (
                                self.config.get("hyperparameter", "lr_scheduler")
                                == "CyclicLR"
                            ):
                                self.scheduler.base_lrs = [
                                    self.optimizer.param_groups[0]["lr"] * 0.2
                                ] * len(self.scheduler.base_lrs)
                                self.scheduler.max_lrs = [
                                    self.optimizer.param_groups[0]["lr"]
                                ] * len(self.scheduler.max_lrs)
                            lr_step += 1
                    with torch.no_grad():
                        val_losses = self.kernel_fn(val_loader, "VAL")
                    print(
                        f"Revert (threshold: {self.config.getfloat('train', 'revert_threshold')}) to epoch {best_checkpoint['epoch']} \t| Val loss: {val_losses.avg:.8f} with lr {self.optimizer.param_groups[0]['lr']:0.2e}"
                    )
                    if self.if_tensorboard:
                        self.tb_writer.add_scalars(
                            "loss",
                            {"Validation loss": val_losses.avg},
                            global_step=epoch,
                        )
                    ...
                    if (
                        ...
                        self.config.get("hyperparameter", "lr_scheduler") == "CyclicLR"
                    ):
                        self.scheduler.step()
                    continue
                if self.if_tensorboard:
                    self.tb_writer.add_scalars(
                        "loss", {"Validation loss": val_losses.avg}, global_step=epoch
                    )
                ...
                while not save_complete:
                    try:
                        save_model(
                            ...
                            path=self.config.get("basic", "save_dir"),
                            is_best=is_best,
                        )
                        save_complete = True
                ...
                if val_losses.avg < self.config.getfloat(
                    "train", "early_stopping_loss"
                ):
                    print(
                        f"Early stopping because the target accuracy (validation loss < {self.config.getfloat('train', 'early_stopping_loss')}) is achieved at eopch #{epoch:01d}"
                    )
                    break
                if (
                    epoch > self.early_stopping_loss_epoch[1]
                    and val_losses.avg < self.early_stopping_loss_epoch[0]
                ):
                    print(
                        f"Early stopping because the target accuracy (validation loss < {self.early_stopping_loss_epoch[0]} and epoch > {self.early_stopping_loss_epoch[1]}) is achieved at eopch #{epoch:01d}"
                    )
                    break
        ...
        best_checkpoint = torch.load(
            os.path.join(self.config.get("basic", "save_dir"), "best_state_dict.pkl")
        )
        self.model.load_state_dict(best_checkpoint["state_dict"])
        ...
        with torch.no_grad():
            ...
            if self.if_tensorboard:
                self.tb_writer.add_scalars(
                    "loss", {"Test loss": test_losses.avg}, global_step=epoch
                )
            test_losses = self.kernel_fn(
                train_loader, tmp, train_csv_name, output_E=True
            )
    ...
    def kernel_fn(self, loader, task: str, save_name=None, output_E=False):
        ...
        for step, batch_tuple in enumerate(loader):
            ...
            if self.target == "E_ij":
                if self.energy_component == "E_ij":
                    ...
                    label_non_onsite = batch.E_delta_ee_ij.to(
                        self.device
                    ) + batch.E_xc_ij.to(self.device)
                    label_onsite = batch.onsite_E_delta_ee_ij.to(
                        self.device
                    ) + batch.onsite_E_xc_ij.to(self.device)
            ...
            if self.target == "E_i":
                ...
                loss_Eij = self.criterion(
                    torch.cat([output_onsite, output_non_onsite], dim=0),
                    torch.cat([label_onsite, label_non_onsite], dim=0),
                )
                output_non_onsite_Ei = scatter_add(
                    output_non_onsite, batch.edge_index.to(self.device)[0, :], dim=0
                )
                label_non_onsite_Ei = scatter_add(
                    label_non_onsite, batch.edge_index.to(self.device)[0, :], dim=0
                )
                ...
                loss_Etot = self.criterion(
                    scatter_add(output_Ei, batch.batch.to(self.device), dim=0),
                    scatter_add(label_Ei, batch.batch.to(self.device), dim=0),
                )
                loss = (
                    loss_Eij * self.lambda_Eij
                    + loss_Ei * self.lambda_Ei
                    + loss_Etot * self.lambda_Etot
                )
            else:
                if self.criterion_name == "MaskMSELoss":
                ...
                if self.config.get("hyperparameter", "optimizer") == "lbfgs":
                    ...
                    if self.config.getboolean("train", "clip_grad"):
                        clip_grad_norm_(
                            ...
                            self.config.getfloat("train", "clip_grad_value"),
                        )
                    self.optimizer.step()
            ...
            if self.target == "E_i" or self.target == "E_ij":
                ...
                if task != "TRAIN" and self.out_fea_len != 1:
                    if self.criterion_name == "MaskMSELoss":
                        ...
                        for index_out, losses_each_out_for in enumerate(
                            losses_each_out
                        ):
                            ...
                            if count == 0:
                                ...
                                losses_each_out_for.update(
                                    torch.masked_select(
                                        se_each_out[:, index_out], mask[:, index_out]
                                    )
                                    .mean()
                                    .item(),
                                    count,
            ...
            if task == "TEST":
                if self.target == "E_ij":
                    ...
                    test_ids += np.array(batch.stru_id)[
                        torch.squeeze(batch.batch).numpy()
                    ].tolist()
                    test_atom_ids += torch.squeeze(
                        torch.tensor(range(batch.num_nodes))
                        - torch.tensor(batch.__slices__["x"])[batch.batch]
                    ).tolist()
                    test_atomic_numbers += torch.squeeze(
                        self.index_to_Z[batch.x]
                    ).tolist()
                elif self.target == "E_i":
                    ...
                    edge_stru_index = torch.squeeze(
                        batch.batch[batch.edge_index[0]]
                    ).numpy()
                    edge_slices = torch.tensor(batch.__slices__["x"])[
                        edge_stru_index
                    ].view(-1, 1)
                    test_preds += torch.squeeze(output.detach().cpu()).tolist()
                    test_targets += torch.squeeze(label.detach().cpu()).tolist()
                    ...
                    test_atom_ids += torch.squeeze(
                        batch.edge_index.T - edge_slices
                    ).tolist()
                    test_atomic_numbers += torch.squeeze(
                        self.index_to_Z[batch.x[batch.edge_index.T]]
                    ).tolist()
                    test_edge_infos += torch.squeeze(
                        batch.edge_attr[:, :7].detach().cpu()
                    ).tolist()
            if output_E is True:
                if self.target == "E_ij":
                    output_non_onsite_Ei = scatter_add(
                        output_non_onsite, batch.edge_index.to(self.device)[1, :], dim=0
                    )
                    label_non_onsite_Ei = scatter_add(
                        label_non_onsite, batch.edge_index.to(self.device)[1, :], dim=0
                    )
                    output_Ei = output_non_onsite_Ei + output_onsite
                    label_Ei = label_non_onsite_Ei + label_onsite
                    Etot_error = (
                    ...
                    for test_stru_id, test_error in zip(batch.stru_id, Etot_error):
                        print(
                            f"{test_stru_id}: {test_error * 1000:.2f} meV / unit_cell"
                        )
                elif self.target == "E_i":
                    Etot_error = (
                        abs(
                            scatter_add(output, batch.batch.to(self.device), dim=0)
                            - scatter_add(label, batch.batch.to(self.device), dim=0)
                        )
                        .reshape(-1)
                        .tolist()
                    )
        ...
        if task != "TRAIN" and (self.out_fea_len != 1):
            ...
            print(
                f"max orbital: {max(loss_list):0.3e} (0-based index: {np.argmax(loss_list)})"
            )
        if task == "TEST":
            with open(
                os.path.join(self.config.get("basic", "save_dir"), save_name),
                "w",
                newline="",
            ) as f:
                writer = csv.writer(f)
                if self.target == "E_i" or self.target == "E_ij":
                    writer.writerow(
                        ["stru_id", "atom_id", "atomic_number"]
                        + ["target"] * self.out_fea_len
                        + ["pred"] * self.out_fea_len
                    )
                    for stru_id, atom_id, atomic_number, target, pred in zip(
                        test_ids,
                        test_atom_ids,
                        test_atomic_numbers,
                        test_targets,
                        ...
                        if self.out_fea_len == 1:
                            writer.writerow(
                                (stru_id, atom_id, atomic_number, target, pred)
                            )
                        else:
                            writer.writerow(
                                (stru_id, atom_id, atomic_number, *target, *pred)
                    ...
                    for stru_id, atom_id, atomic_number, edge_info, target, pred in zip(
                        test_ids,
                        test_atom_ids,
                        test_atomic_numbers,
                        test_edge_infos,
